---
layout: post
title: "关于《动手学深度学习（第二版）》学习笔记系列的目录"
date: 2025-10-27
tags: [AI, catalogue]
toc: false
comments: true
author: pianfan
---

本系列为博主在学习[《动手学深度学习（第二版）》](https://zh.d2l.ai/)过程中记录的笔记。<!-- more -->由于中文版有些地方翻译得有些晦涩，博主在阅读的过程中同时参考了[英文版](https://d2l.ai/)的表述。本系列笔记主要记录学习过程中遇到的核心概念与相关的数学和代码表示，以便今后在需要用到时可以通过笔记快速回顾。

## 系列目录

- [1. 引言](https://pianfan.github.io/d2l_introduction/)

- [2. 预备知识](https://pianfan.github.io/d2l_preliminaries/)

- [3. 线性神经网络](https://pianfan.github.io/d2l_linear-networks/)

后续章节持续更新中……
